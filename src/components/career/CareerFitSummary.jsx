...
export default function CareerFitSummary({ aptitude = {}, language = "en" }) {
  const [userAptitude, setUserAptitude] = useState(null);

  // Fetch user aptitude from API
  useEffect(() => {
    async function fetchScores() {
      try {
        ...
        res?.data?.data
          ...
          .forEach((s) => {
            apt[s.traitOrCategoryCode] = s.score;
          });

        setUserAptitude(apt);
      } catch (e) {
        console.error("Error fetching user aptitude:", e);
      }
    }
  ...
  if (!aptitude || !userAptitude) return null;
    ...
    <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
      ...
      <div className="space-y-6">
        ...
        {Object.entries(aptitude).map(([key, idealScore]) => {
          const userScore = userAptitude[key] ?? 0;
